...
def _linux_evidence(d: str, fixture_dir: str) -> tuple[dict, dict]:
    ...
    if (
        recipe["family"] != "linux"
        or profile["id"] != "linux-glibc-x86_64-loose-v1"
    ):
...
def _linux_readings(d: str, join: dict) -> list:
    ...
    for file in elf_files:
        with open(file.path, "rb") as stream:
            ...
            ).data().rstrip(b"\x00").decode("ascii")
            dynamic = next(
                segment for segment in elf.iter_segments()
                if segment.header.p_type == "PT_DYNAMIC"
            )
            needed = [
                tag.needed for tag in dynamic.iter_tags()
    ...
    filesystem.map_file_fh(
        "/etc/passwd",
        BytesIO(
            f"{join['user']}:x:1000:1000:ArtifactForge:"
            f"{join['home_dir']}:/bin/bash\n".encode()
        ),
    )
