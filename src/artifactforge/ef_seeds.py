...
recovering which Sysmon-local seed input each verified hash denotes.
...
class Identity:
    """One verified Sysmon-local seed identity and its ArtifactForge materialization key."""

    # Stable only as an ArtifactForge input for this recovered Sysmon seed. It is not an
    # EvidenceForge cross-emitter identity and does not claim the generated bytes were observed.
    content_id: str
...
def recover(emitted_sha256: str, *, image: str, file_version=None, description=None,
    ...
    """Recover which Sysmon-local seed input an emitted hash denotes, or raise.
